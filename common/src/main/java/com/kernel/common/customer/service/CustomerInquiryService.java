...
public interface CustomerInquiryService {
    ...
    // 수요자 문의사항 검색 및 조회
    Page<CustomerInquiryRspDTO> searchCustomerInquiries(Long customerId, LocalDateTime startDate, Pageable pageable);
    ...
    // 수요자 문의사항 수정
    CustomerInquiryDetailRspDTO updateCustomerInquiry(Long customerId, Long inquiryId, CustomerInquiryUpdateReqDTO inquiryRequestDTO);
  
    // 수요자 문의사항 삭제
    void deleteCustomerInquiry(Long customerId, Long inquiryId);

    // 수요자 문의사항 카테고리 조회
    List<InquiryCategoryRspDTO> getCustomerInquiryCategory();
}
